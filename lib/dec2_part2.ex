defmodule Dec2Part2 do
  ...
  def brute_force do
    ...
    |> Enum.each(fn noun ->
      ...
      |> Enum.each(fn verb ->
        output =
          program |> Map.put(1, noun) |> Map.put(2, verb) |> Dec2.run_program(0) |> Map.get(0)

        case output do
          19_690_720 ->
            IO.puts(
              "noun #{noun} and verb #{verb} produce output 19690720, 100 * noun + verb = #{100 * noun + verb}"
            )
