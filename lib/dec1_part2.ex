defmodule Dec1Part2 do
  ...
  --- Part Two ---
    ...
    A module of mass 14 requires 2 fuel. This fuel requires no further fuel (2 divided by 3 and rounded down is
    0, which would call for a negative fuel), so the total fuel required is still just 2.
  ...
  def run do
    {:ok, contents} = File.read("lib/inputs/dec1.txt")

    fuel_required(contents)
  end

  def fuel_required(rocket_modules) do
    rocket_modules \
      |> String.split("\n", trim: true)
      |> Enum.map(fn n -> Float.parse(n) |> elem(0) end)
      |> Enum.reduce(0, &fuel_required_per_module/2)
  end

  def fuel_required_per_module(fuel_mass, accum) when fuel_mass / 3 > 2  do
    required_mass = Float.floor(fuel_mass / 3) - 2
    fuel_required_per_module(required_mass, accum + required_mass)
  end

  def fuel_required_per_module(_fuel_mass, accum) do
    accum
  end
end
