defmodule Dec2 do
  ...
  def run(string) do
    ...
    |> run_program(_initial_position = 0)
    |> Map.values()
    |> Enum.join(",")
  end

  def run_program(program, index) do
    op = Map.get(program, index, 99)

    case op do
      1 ->
        new_program = perform_action(&+/2, program, index)
        run_program(new_program, index + 4)
  ...
  defp perform_action(op, map, index) do
    val1 = Map.get(map, Map.get(map, index + 1))
    val2 = Map.get(map, Map.get(map, index + 2))
    position = Map.get(map, index + 3)

    Map.put(map, position, op.(val1, val2))
  end

  def build_program(string) do
    string # "1,0,0,0,99"
    |> String.split(",", trim: true) # ["1","0","0","0","99"]
    |> Enum.map(fn n -> Integer.parse(n) |> elem(0) end) # [1,0,0,0,99]
    |> Enum.with_index() # [1:0, 0:1, 0:2, 0:3, 99:4]
    |> Enum.map(&invert/1) # [0:1, 1:0, 2:0, 3:0, 4:99]
    |> Enum.reduce(%{}, &mapify/2) # %{0=>1, 1=>0, 2=>0, 3=>0, 4=>99}
  end
